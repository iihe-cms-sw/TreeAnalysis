...
void FuncUnfold(string variable,  int UsedKtermBayes, int UsedKtermSVD, bool doFlat , bool doVarWidth )
{
   ...
   UsedKterm = 150 ;
    ...
    for ( int i = 0; i < 2 ; i++){
        ...
        getFiles(FILESDIRECTORY, fData[i], leptonFlavor, energy, ProcessInfo[DATAFILENAME].filename, JetPtMin, JetPtMax, doFlat,  doVarWidth, 0, 0, 0, 0, 1);
        ...
        getHistos(hDYT, fDYMadGraph, variable);
        getHistos(hDYOriginal[i], fDYMadGraph, variable);
        if ( i == 1 ) hDYOriginal[i][0]->Draw("sranje");
        hDYGenMadGraphT = getHisto(fDYMadGraph[0], "gen" + variable);
        hDYGenSherpaT = getHisto(fDYSherpa, "gen" + variable);
        hDYGenPowhegT = getHisto(fDYPowheg, "gen" + variable);
        hGen[i][0] = getHisto(fDYMadGraph[i], "gen" + variable);
        ...
        cout <<__LINE__ << endl;
        getResps(resDYT, fDYMadGraph, "response" + variable);
        getResps(resDYOriginal[i], fDYMadGraph, "response" + variable);
        ...
        for ( int l = 0 ; l <  NFilesMC ; l++){
            if (    hDYT[l]->GetBinContent(NBins + 1) <  MinNumberInBin ) NBins = hDYT[l]->GetNbinsX() ;
        }
    ...
    for (int i(0); i < nFiles; i++){
        ...
        for ( int k = 0; k < 2 ; k++){
            ...
            for (int j(0); j < NBkgSyst ; j++){
                ...
                else hSumBGTmp[k][j]->Add(hBGTmp[k][countFiles][j]);
                cout << "usbgroup " << fileNameTemp <<"  " << group[k] <<"   " <<  j << "   " << countFiles << "   " << countBGp[k][1] <<endl;
    ...
    //
    hDataOrig[0][0]->Write("data muon");
    hDataOrig[1][0]->Write("data ele");
    hGen[0][0]->Write("gen muon");
    hGen[1][0]->Write("gen ele");
    ((TH1D*) resDYOriginal[0][0]->Htruth())->Write("truth muon");
    ((TH1D*) resDYOriginal[1][0]->Htruth())->Write("truth ele");
   ...
   plotChi2OfChange(responseNEW[0], 200 , (TH1D*) hData[0]->Clone(),  hSumBG, 1, "Combine_", variable, energy, outputDirectory, outputRootFile, 0 );
