		...
		bool doRoch = 0, bool doFlat = 0, bool doVarWidth = 1)
{
	...
	if ( leptonFlavor == "SMuE" || leptonFlavor == "SMu" || leptonFlavor == "Muon") {
		 nFiles = NfilesTTbar ; 
		 string legendNamesTTbar[] = {
                " #mue ", " ZZ",
                " WZ", " WW", " WJets"," QCD", " Single Top",
                " TTJets", " DYJets"
        	};	
		if ( leptonFlavor == "SMu" ) legendNamesTTbar[0]="#mu#nu";		 
		if ( leptonFlavor == "SMuE" && ( doQCD > 0 || doInvMassCut) ) {
		legendNamesTTbar[5] =" Single Top";
		legendNamesTTbar[6] =" TTJets";
		legendNamesTTbar[7] =" DYJets";

		}
		std::copy( FILENAMESTTbar, FILENAMESTTbar + nFiles, FILENAMES );
		std::copy( legendNamesTTbar, legendNamesTTbar + nFiles, legendNames );
	...
	for (unsigned short i = 0; i < nFiles; i++){
		...
		if ( (  doQCD > 0  || doInvMassCut) && FILENAMES[i].find("QCD") != -1 ) continue;
		file[countF] = getFile(FILESDIRECTORY, leptonFlavor, energy, FILENAMES[i], JetPtMin, JetPtMax, doFlat, doVarWidth, doQCD, doInvMassCut);
		for ( int j = 0 ; j < NFilesFull ; j++){
			if ( ProcessInfo[j].filename == FILENAMES[i] ) {
					legendNames[countF] = ProcessInfo[j].legend;
					Colors[countF] = ProcessInfo[j].color;										
			}
		}
		cout << FILENAMES[i] <<"   " << legendNames[countF] << "  " << Colors[countF] << endl;
		countF++;
	}
	nFiles = countF ;
	//-----------------------------------------------------
	...
	//nHist=4;
	//int Colors[] = {kBlack, kSpring+5, kOrange, kOrange-3, kRed+1, kPink-6, kViolet+5, kPink, kAzure+4, kBlue, kCyan+1, kCyan+1, kCyan+1}; 
	if ( leptonFlavor == "SMuE") { 
		int ColorsTTbar[] = {kBlack, kOrange,  kRed+1,  kViolet+5, kPink, kBlue, kCyan+1, kCyan+1, kCyan+1};
		//std::copy( ColorsTTbar, ColorsTTbar + nFiles, Colors );
		}
