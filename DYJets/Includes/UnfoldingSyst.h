...
TH1D * ToyMCErrorsStat( string unfAlg , TH1D *hdata, TH1D* hBack0[], RooUnfoldResponse *response, int kterm, TH2D *hCovariance, TH2D *hCorrelation, int nPseudos , int selection  ){
  ...
  if (selection == 1 ) { fluctPData = true; fluctRMat = true; fluctBkg = true; usePoisson = true; }
  // PU effect 
  ...
  // output histogram
  const int NBins(hTrue1->GetNbinsX());
  ...
  for(int i(0); i < nPseudos; i++){
    ...
    if(nPseudos > 1) {
      ...
      if ( selection == 1 || selection == 3 ) {
        for (int j(1); j <= h_meas->GetNbinsX(); j++) {
          double fluct(0);
          double BinCount( hdata->GetBinContent(j) );
          double BinError( sqrt(BinCount) );
          if ( hdata->GetBinError(j) > 0 ) BinError = hdata->GetBinError(j);
          double ratio = (BinError * BinError )/BinCount ;
          if (BinCount > 0 && usePoisson ) fluct = random->Poisson(BinCount/ratio);
          ...
          //cout << "poisson errors on data : " << i << "   " << BinCount << "   " <<BinError << "     " << fluct * ratio << endl;
        }
      }
      ...
      if ( ( selection == 1 || selection == 2 || selection == 4 ) && fluctRMat == true){
        //Poisson fluctuate the R Matrix
        for (int j(1); j <= h_meas->GetNbinsX(); j++){
          for (int k(1); k <= hTrue1->GetNbinsX(); k++){
      ...
      //independent Gauss and Poisson fluctuate the backgrounds
      if ( ( selection == 1 || selection == 2 || selection == 5 ) && fluctBkg == true) {
        for (int j(1); j <= h_bkg->GetNbinsX(); j++) {
          ...
          //cout<<" backgroun MC Toy:  "<< BinCount << " new value" <<fluctB<<" data  "<<hdata->GetBinContent(j)<<endl;
        }
      }
      ...
      if ( ( selection == 12 || selection == 11)  && fluctBkg == true) {
        ...
        for (int k(0); k <= 3 ; k++) {
          ...
          for (int j(1); j <= h_bkg->GetNbinsX(); j++) {
            ...
            if (fluctBtemp > 0) fluctB[j] += fluctLUMIsig * fluctBtemp;
          }
        }
        for (int j(1); j <= h_bkg->GetNbinsX(); j++) {
