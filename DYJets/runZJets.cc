...
int main(int argc, char **argv)
{
    ...
    if (doWhat == "DATA" || doWhat == "ALL") {
        ...
        for (unsigned int i(start); i < NSystData; i++) {
            ...
            ZJets DMudata(lepSel + "_8TeV_Data_dR", 1, 1, dataSyst[i], dataDir[i], 1, jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            DMudata.Loop(hasRecoInfo, hasGenInfo);
            //ZJets DMudataA(lepSel + "_8TeV_Data_dR_RunA", 1, 1, dataSyst[i], dataDir[i], 1, jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            //DMudataA.Loop(hasRecoInfo, hasGenInfo);
            //ZJets DMudataB(lepSel + "_8TeV_Data_dR_RunB", 1, 1, dataSyst[i], dataDir[i], 1, jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            //DMudataB.Loop(hasRecoInfo, hasGenInfo);
            //ZJets DMudataC(lepSel + "_8TeV_Data_dR_RunC", 1, 1, dataSyst[i], dataDir[i], 1, jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            //DMudataC.Loop(hasRecoInfo, hasGenInfo);
            //ZJets DMudataD(lepSel + "_8TeV_Data_dR_RunD", 1, 1, dataSyst[i], dataDir[i], 1, jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
    ...
    if (doWhat == "BACKGROUND" || doWhat == "ALL") {
        ...
        for (unsigned int i(start); i < NSystMC; i++) { 
            ...
            ZJets ZZ(lepSel + "_8TeV_ZZJets2L2Nu_dR",        lumi*17.654*0.04039 *1000/954911.,  1, bgSyst[i], bgDir[i], bgScale[i], jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            ZZ.Loop(hasRecoInfo, hasGenInfo);
            ZJets WW(lepSel + "_8TeV_WWJets2L2Nu_dR",        lumi*54.838*0.10608 *1000/1933235., 1, bgSyst[i], bgDir[i], bgScale[i], jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            WW.Loop(hasRecoInfo, hasGenInfo);
            ZJets ZZ1(lepSel + "_8TeV_ZZJets2L2Q_dR",        lumi*17.654*0.14118 *1000/1936727., 1, bgSyst[i], bgDir[i], bgScale[i], jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            ZZ1.Loop(hasRecoInfo, hasGenInfo);
            ZJets ZZ2(lepSel + "_8TeV_ZZJets4L_dR",          lumi*17.654*0.010196*1000/4807893., 1, bgSyst[i], bgDir[i], bgScale[i], jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            ZZ2.Loop(hasRecoInfo, hasGenInfo);
            ZJets WZ(lepSel + "_8TeV_WZJets3LNu_dR",         lumi*33.21 *0.032887*1000/2017979., 1, bgSyst[i], bgDir[i], bgScale[i], jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            WZ.Loop(hasRecoInfo, hasGenInfo);
            ZJets WZ1(lepSel + "_8TeV_WZJets2L2Q_dR",        lumi*33.21 *0.068258*1000/3215990., 1, bgSyst[i], bgDir[i], bgScale[i], jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            WZ1.Loop(hasRecoInfo, hasGenInfo);
            ZJets T(lepSel + "_8TeV_T_s_channel_dR",         lumi*3.79           *1000/259961.,  1, bgSyst[i], bgDir[i], bgScale[i], jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            T.Loop(hasRecoInfo, hasGenInfo);
            ZJets T1(lepSel + "_8TeV_T_t_channel_dR",        lumi*56.4           *1000/3747932., 1, bgSyst[i], bgDir[i], bgScale[i], jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            T1.Loop(hasRecoInfo, hasGenInfo);
            ZJets T2(lepSel + "_8TeV_T_tW_channel_dR",       lumi*11.1           *1000/497658.,  1, bgSyst[i], bgDir[i], bgScale[i], jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            T2.Loop(hasRecoInfo, hasGenInfo);
            ZJets Tbar(lepSel + "_8TeV_Tbar_s_channel_dR",   lumi*1.76           *1000/139974.,  1, bgSyst[i], bgDir[i], bgScale[i], jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            Tbar.Loop(hasRecoInfo, hasGenInfo);
            ZJets Tbar1(lepSel + "_8TeV_Tbar_t_channel_dR",  lumi*30.7           *1000/1935072., 1, bgSyst[i], bgDir[i], bgScale[i], jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            Tbar1.Loop(hasRecoInfo, hasGenInfo);
            ZJets Tbar2(lepSel + "_8TeV_Tbar_tW_channel_dR", lumi*11.1           *1000/493460.,  1, bgSyst[i], bgDir[i], bgScale[i], jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            Tbar2.Loop(hasRecoInfo, hasGenInfo);
            ZJets TT(lepSel + "_8TeV_TTJets_dR",             lumi*245.           *1000/6923750., 1, ttSyst[i], ttDir[i], ttScale[i], jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            TT.Loop(hasRecoInfo, hasGenInfo);

            // Additionaly merge the top samples
            runMergeTop(lepSel, bgSyst[i]*bgDir[i], jetPtMin, jetEtaMax, maxEvents, histoDir);
        }
    }
    ...
    if (doWhat == "TAU" || doWhat == "ALL") {
        ...
        for (unsigned int i(start); i < NSystMC; i++) { 
            if (i == 0 && !doCentral && whichSyst < 0) continue;
            ZJets DYTau(lepSel + "_8TeV_DYJetsToLL_FromTau_50toInf_UNFOLDING_dR", lumi*3531.8*1000/30459503., 1, tauSyst[i], tauDir[i], tauScale[i], jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
    ...
    if (doWhat == "DYJETS" || doWhat == "ALL") {
        ...
        for (unsigned int i(start); i < NSystMC; i++) { 
            if (i == 0 && !doCentral && whichSyst < 0) continue;
            ZJets DYMix(lepSel + "_8TeV_DYJetsToLL_MIX_50toInf_UNFOLDING_dR", lumi*3531.8*1000/30459503., 1, dySyst[i], dyDir[i], 1, jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
            DYMix.Loop(hasRecoInfo, hasGenInfo);
            //ZJets DY(lepSel + "_8TeV_DYJetsToLL_50toInf_UNFOLDING_dR", lumi*3531.8*1000/30459503., 1, dySyst[i], dyDir[i], 1, jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
    ...
    if (doWhat == "WJETS" || doWhat == "ALL") {
        ...
        for (unsigned int i(start); i < NSystMC; i++) { 
            if (i == 0 && !doCentral && whichSyst < 0) continue;
            ZJets WJMix(lepSel + "_8TeV_WJetsALL_MIX_UNFOLDING_dR", lumi*36703.         *1000/76102995., 1, wjSyst[i], wjDir[i], wjScale[i], jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
    ...
    if (doWhat == "PDF") {
        for (int pdfMember(0); pdfMember <= 0; pdfMember++) {
            ZJets DYMixPDF(lepSel + "_8TeV_DYJetsToLL_MIX_50toInf_UNFOLDING_dR", lumi*3531.8*1000/30459503., 1, 0, 0, 1, jetPtMin, jetEtaMax, maxEvents, histoDir, bonzaiDir);
