{
  string incdir = "/user/tseva/analysis/TreeAnalysis2012/DYJets/Includes/";
...
//    gSystem->Load(string(srcdir +sources[i] + "_cc.so").c_str());
   gROOT->ProcessLine(string(".L " + srcdir + sources[i] + ".cc+").c_str());
  }
//for (unsigned int i(0); i < nSources; i++) gROOT->ProcessLine(string(".L " + srcdir + sources[i] + ".cc+").c_str());
  //gROOT->ProcessLine(".L /usr/local/lib/libLHAPDF.dylib");

  double  muLumi(19.549); // DoubleMu number with pixelCalc
  double eleLumi(19.602); // DoubleMu number with pixelCalc
 ...
 //         DMuDY.Loop(1, 0,  1,  1, 1, 22, doBJets, 9 );
    ZJetsAndDPS DEDY("DE_8TeV_DYJets_UNFOLDING_dR_5311_Inf3",  muLumi*3531.8*1000/30459503., 1., 1, !doDataEff, 0, 0, 1,          jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax);
///          DEDY.Loop(1, 1,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );
  ZJetsAndDPS DMuTTrew(lepSelection+"_8TeV_TTJets_dR_5311_TopReweighting",             muLumi*245.           *1000/6923652., 1, 1, !doDataEff, ttSyst[i], ttDir[i], ttScale[i], jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax);
            DMuTTrew.Loop(1, 1, doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );

  }
  if ( doWhat == 0 || doWhat == 100 ){
	  for (unsigned int i(0); i < NSystData; i++){
		  ZJetsAndDPS DMudata(lepSelection+"_8TeV_Data_dR_5311",             1.,                                   1, 1, doDataEff, dataSyst[i], dataDir[i], 1, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax );
  ...
  if ( doWhat == 1 || doWhat == 100 ){
	  for (unsigned int i(0); i < NSystMC; i++){ 
		  ...
		  if ( (lepSelection.find("SE") == -1 && lepSelection.find("SMu") == -1 )  ) {
		          ZJetsAndDPS DMuWJ(lepSelection+"_8TeV_WJetsALL_UNFOLDING_dR_5311",           muLumi*36703.         *1000/76102995.,1, 1, !doDataEff, bgSyst[i], bgDir[i], bgScale[i], jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax);
		          DMuWJ.Loop(1, 0, doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );

                  ZJetsAndDPS DMuWJmix(lepSelection+"_8TeV_WJetsALL_MIX_UNFOLDING_dR_5311",           muLumi*36703.         *1000/76102995.,1, 1, !doDataEff, bgSyst[i], bgDir[i], bgScale[i], jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax);
  ...
  if ( doWhat == 2 || doWhat == 100 ){
	  for (unsigned int i(3); i < 5; i++){ 
		  ZJetsAndDPS DMuDYTau(lepSelection+"_8TeV_DYJets_FromTau_UNFOLDING_dR_5311_Inf3", muLumi*3531.8*1000/3045950, 1., 1,  !doDataEff, tauSyst[i], tauDir[i], tauScale[i], jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax);
  ...
  if ( doWhat == 3 || doWhat == 100 ){
	  ...
	  ZJetsAndDPS DMuDYTauS(lepSelection+"_8TeV_DYJets_FromTau_UNFOLDING_dR_5311_Inf3", muLumi*3531.8*1000/30459503., 1, 1, !doDataEff, 0, 0, 1, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax);
//	  DMuDYTauS.Loop(1, 1,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );

	  ZJetsAndDPS DMuDY(lepSelection+"_8TeV_DYJets_UNFOLDING_dR_5311_Inf3",  muLumi*3531.8*1000/30459503., 1., 1, !doDataEff, 0, 0, 1, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax);
//	  DMuDY.Loop(1, doGen,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );
	  ZJetsAndDPS DMuDYMix(lepSelection+"_8TeV_DYJets_MIX_UNFOLDING_dR_5311_Inf3", muLumi*3531.8*1000/30459503., 1., 1, !doDataEff, 0, 0, 1, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax, 0);
//	  DMuDYMix.Loop(1, doGen,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );
	  if ( doSysRunning){
		  //PU
		  ZJetsAndDPS DMuDYMix1(lepSelection+"_8TeV_DYJets_MIX_UNFOLDING_dR_5311_Inf3", muLumi*3531.8*1000/30459503., 1., 1, !doDataEff, 1, 1, 1, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax, 0);
		  DMuDYMix1.Loop(1, doGen,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );
		  ZJetsAndDPS DMuDYMix2(lepSelection+"_8TeV_DYJets_MIX_UNFOLDING_dR_5311_Inf3", muLumi*3531.8*1000/30459503., 1., 1, !doDataEff, 1,-1, 1, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax, 0);
		  DMuDYMix2.Loop(1, doGen,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );

		  // jet smearing part
		  if ( doGen ) { 
			  ZJetsAndDPS DMuDYMix3(lepSelection+"_8TeV_DYJets_MIX_UNFOLDING_dR_5311_Inf3", muLumi*3531.8*1000/30459503., 1., 1, !doDataEff, 4, 1, 1, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax, 0);
			  DMuDYMix3.Loop(1, doGen,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );
		  }
		  else { // xsec offsets
			  ZJetsAndDPS DMuDYMix4(lepSelection+"_8TeV_DYJets_MIX_UNFOLDING_dR_5311_Inf3", muLumi*3531.8*1000/30459503., 1., 1, !doDataEff, 3, 1, 0.04, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax, 0);
			  DMuDYMix4.Loop(1, doGen,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );

			  ZJetsAndDPS DMuDYMix5(lepSelection+"_8TeV_DYJets_MIX_UNFOLDING_dR_5311_Inf3", muLumi*3531.8*1000/30459503., 1., 1, !doDataEff, 3, -1, 0.04, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax, 0);
			  DMuDYMix5.Loop(1, doGen,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );

		  } 
		  ZJetsAndDPS DMuDYMix6(lepSelection+"_8TeV_DYJets_MIX_UNFOLDING_dR_5311_Inf3", muLumi*3531.8*1000/30459503., 1., 1, !doDataEff, 1, 1, 1, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax, 0);
		  DMuDYMix6.Loop(1, doGen,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );
		  ZJetsAndDPS DMuDYMix7(lepSelection+"_8TeV_DYJets_MIX_UNFOLDING_dR_5311_Inf3", muLumi*3531.8*1000/30459503., 1., 1, !doDataEff, 1,-1, 1, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax, 0);
...
if ( doWhat == 4 || doWhat == 100 ){
	...
	ZJetsAndDPS DMuWJ(lepSelection+"_8TeV_WJetsALL_UNFOLDING_dR_5311",  muLumi*36703.         *1000/76102995., 1., 1, !doDataEff, 0, 0, 1, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax);
	DMuWJ.Loop(1, doGen,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );
	ZJetsAndDPS DMuWJMix(lepSelection+"_8TeV_WJetsALL_MIX_UNFOLDING_dR_5311", muLumi*36703.         *1000/76102995., 1., 1, !doDataEff, 0, 0, 1, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax, 0);
	DMuWJMix.Loop(1, doGen,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );

          if ( doSysRunning){
                  //PU
                  ZJetsAndDPS DMuWJMix(lepSelection+"_8TeV_WJetsALL_UNFOLDING_dR_5311", muLumi*36703*1000/76102995., 1., 1, !doDataEff, 1, 1, 1, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax, 0);
                  DMuWJMix.Loop(1, doGen,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );
                  ZJetsAndDPS DMuWJMix(lepSelection+"_8TeV_WJetsALL_UNFOLDING_dR_5311", muLumi*36703*1000/76102995., 1., 1, !doDataEff, 1,-1, 1, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax, 0);
                  DMuWJMix.Loop(1, doGen,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );
                  ...
                  // jet smearing part
                  if ( doGen ) {
                          ZJetsAndDPS DMuWJMix(lepSelection+"_8TeV_WJetsALL_UNFOLDING_dR_5311", muLumi*36703*1000/76102995., 1., 1, !doDataEff, 4, 1, 1, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax, 0);
                          DMuWJMix.Loop(1, doGen,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );
                  }
                  else { // xsec offsets
                          ZJetsAndDPS DMuWJMix(lepSelection+"_8TeV_WJetsALL_UNFOLDING_dR_5311", muLumi*36703*1000/76102995., 1., 1, !doDataEff, 3, 1, 0.04, jetPtMin, jetPtMax, ZEtaMin,    ZEtaMax, 0);
                          DMuWJMix.Loop(1, doGen,  doQCD,  doSSign, doInvMassCut, doBJets, doPUStudy );
...
//	DESherpaTest1NLOScaleDown.Loop(0, 1, 0, 0); 
  ...
  //      DESherpaTest1NLOScaleUp.Loop(0, 1, 0, 0); 
	...
	//ZJetsAndDPS DMuSherpa(lepSelection+"_DYJets_Sherpa_mcEveWeight",   muLumi * 3531.8           * 1000 / 30459503.,    1.,  0,  0,  0,  0,  1.,  jetPtMin,  jetPtMax, ZEtaMin,    ZEtaMax );
